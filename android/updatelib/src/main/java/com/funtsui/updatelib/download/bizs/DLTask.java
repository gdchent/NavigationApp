...
class DLTask implements Runnable, IDLThreadListener {
    ...
    public void run() {
        ...
        while (info.redirect < DLCons.Base.MAX_REDIRECTS) {
            ...
            try {
                ...
                switch (code) {
                    case DLCons.Code.HTTP_OK:
                    case DLCons.Code.HTTP_PARTIAL:
                        dlInit(conn, code);
                        return;
                    case DLCons.Code.HTTP_MOVED_PERM:
                    case DLCons.Code.HTTP_MOVED_TEMP:
                    case DLCons.Code.HTTP_SEE_OTHER:
                    case DLCons.Code.HTTP_NOT_MODIFIED:
                    case DLCons.Code.HTTP_TEMP_REDIRECT:
                ...
                if (info.hasListener)
                    info.listener.onError(DLError.ERROR_OPEN_CONNECT, e.toString());
                DLManager.getInstance(context).removeDLTask(info.baseUrl);
                return;
            } finally {
    ...
    private void dlDispatch() {
        int threadSize;
        int threadLength = DLCons.Base.LENGTH_PER_THREAD;
        if (info.totalBytes <= DLCons.Base.LENGTH_PER_THREAD) {
            threadSize = 2;
            threadLength = info.totalBytes / threadSize;
        } else {
            threadSize = info.totalBytes / DLCons.Base.LENGTH_PER_THREAD;
        }
